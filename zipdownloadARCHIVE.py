# -*- coding: utf-8 -*-
"""
File to download zip files from http://www.nemweb.com.au/REPORTS/ARCHIVE/Yesterdays_Bids_Reports/
...
url = 'http://www.nemweb.com.au/REPORTS/ARCHIVE/Yesterdays_Bids_Reports/' #url path to the main website
...
for links in target: 
    if links != 'http://www.nemweb.com.au/REPORTS/ARCHIVE/':
        #link = links[80:links[80:].index(.)] # could figure out a way to store
    #then could delete the zip files of the machine
        if links[65:] not in downloads:
            targeturl.retrieve(links, links[65:]) # zip file download
            ZipFile(links[65:],"r").extractall(os.getcwd()) #extracting csv from zip
            #os.remove(links[80:]) #removing file from computer
            print links[65:] + " downloaded"
